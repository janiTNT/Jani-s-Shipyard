...
PIX_OUTPUT pix(in VERT_OUTPUT_CHAINGUN input) : SV_TARGET
{
	...
	float3 bluuue = float3(0, 1, 1);

	float3 col = _color2.rgb * zoomBullet + float3(_color2.r * zoomBulletRed, 1 * zoomBulletRed, 1 * zoomBulletRed) + _color1.rgb * mask + bluuue * zoomBulletYellow;
	return float4(col, 1);
