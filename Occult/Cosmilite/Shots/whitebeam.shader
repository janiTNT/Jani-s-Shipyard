...
VERT_OUTPUT_BEAM vert(in VERT_INPUT_BEAM input)
{
	...
	float scrollSpeed = -8 * input.beamTime;
...
PIX_OUTPUT pix(in VERT_OUTPUT_BEAM input) : SV_TARGET
{
	...
	float4 colorA = float4(0.5, 0.5, 0.5, 1); //desired color * 0.5
	...
	float4 col = lerp(light, layer1, innerGrad); //blending in light using a cylinder gradient
	col = lerp(light * noiseBase, col, innerBeam); //breaking up the outline
