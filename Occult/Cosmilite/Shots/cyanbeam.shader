...
PIX_OUTPUT pix(in VERT_OUTPUT_BEAM input) : SV_TARGET
{
	...
	float4 colorA = float4(0.15, 0.29, 0.5, 1); //desired color * 0.5
	float4 cyan = float4(0, 0.5, 1, 1);
	float4 layer1 = colorA * innerCombined * innerSharp; //tinting the processed noise and masking it with innerSharp
	float4 col = lerp(cyan, layer1, innerGrad); //blending in cyan using a cylinder gradient
	col = lerp(cyan * noiseBase, col, innerBeam); //breaking up the outline
