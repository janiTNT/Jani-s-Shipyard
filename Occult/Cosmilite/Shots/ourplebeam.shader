...
PIX_OUTPUT pix(in VERT_OUTPUT_BEAM input) : SV_TARGET
{
	...
	float4 colorA = float4(0.5, 0.15, 0.29, 1); //desired color * 0.5
	float4 purp = float4(1, 0, 0.5, 1);
	float4 layer1 = colorA * innerCombined * innerSharp; //tinting the processed noise and masking it with innerSharp
	float4 col = lerp(purp, layer1, innerGrad); //blending in purp using a cylinder gradient
	col = lerp(purp * noiseBase, col, innerBeam); //breaking up the outline
